...
The \SI profiles (top row of Figure \ref{fig: slices_grid}) show a single peak at the centre of the disk at all four wavelengths. This is expected, as the density of the disk increases towards the central star, resulting in stronger emission from the inner disk. The angular resolution of these data is not sufficient to resolve the rings seen in \cite{Dom2020} (Figure \ref{fig: IRS63Dom2024}). However, in the debiased polarized intensity profiles (bottom row of Figure \ref{fig: slices_grid}), we do not see the same trend. The profiles are consistently offset from the centre position at all four wavelengths. Relative to the synthesized beam, the offset is not well resolved.  Nevertheless, having the offset is consistent across all four wavelengths indicates it is a real feature.
