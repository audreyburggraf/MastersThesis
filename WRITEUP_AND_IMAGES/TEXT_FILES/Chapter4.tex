...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.925\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/StokesI_POLI/StokesI_grid_robust_minus1_Delta_stretched.pdf}

    \vspace{0.5cm}

    \includegraphics[width=0.925\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/StokesI_POLI/POLI_grid_robust_minus1_Delta.pdf}
...
\noindent where $\theta_{\text{obs}}$ is the observed polarization angle and $\theta_{\text{model}}$ is the polarization angle predicted by either the ratio or Gaussian model. 

The angular differences were then used to calculate the reduced chi-squared value:
\begin{equation}
    \chi^2_{reduced} = \frac{1}{n-k} \times \sum \left(\frac{\Delta \theta_i}{\sigma_\theta}\right)^2,
    \label{eq: chi_sq_reduced}
\end{equation}
\noindent where the angular distance is summed over all sampled polarization vectors, and \PAerr is the uncertainty in the observed polarization angle. Additionally, $n$ is the number of sampled polarization vectors and $k$ is the degrees of freedom. For the ratio model (Chapter \ref{sec: ch4_ratio_model}), there is one degree of freedom, and for the Gaussian method (Chapter \ref{sec: ch4_gaussian_model}), there are three ($\phi$, $b$, and $a$).  
...
\begin{figure}[htbp]
  ...
  \caption[Same as Figure \ref{fig: RatioBestGrid}, but for the best-fit Gaussian models.]{Same as Figure \ref{fig: RatioBestGrid}, but for the best-fit Gaussian models. The corresponding best-fit Gaussian models are listed in Table \ref{tab:gaussian_top5}. }
...
\end{equation}
\noindent A larger value of $A$ corresponds to a larger area of uniform morphology. For \bfourNS, \bfiveNS, \bsixNS, and \bseven the corresponding areas are 200, 600, 900, and 1200 pixels$^2$, respectively. The increase in area as wavelength decreases indicates that the uniform morphology contributes over a larger region of the disk at shorter wavelengths. This can be seen in Figure \ref{fig: gaussian_best_map_w_uniform}, showing the Gaussian shape ($W_{\text{Uniform}}$) for the best-fit model at each wavelength.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/Gaussian/Best_grid_Delta_map.pdf}
  \caption[Maps of the Gaussian uniform weights for the best-fit Gaussian model.]{Maps of the Gaussian uniform weights $W_{\text{uniform}}$ (Equation \ref{eq: gaussian_eq}) for the best-fit Gaussian model for each wavelength, with the corresponding vector pattern shown in Figure \ref{fig: GaussianBestGrid}. }
