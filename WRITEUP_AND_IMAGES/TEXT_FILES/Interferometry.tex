...
\begin{equation}
    \theta_{\text{res}}\sim \frac{\lambda}{D},
\end{equation}
...
\begin{equation}
    \theta_{\text{res}}\sim \frac{\lambda}{B}, 
\end{equation}
...
\begin{figure}[htbp]
  ...
  \caption[Interferometer geometry and coordinates]{Schematic showing the geometric relationship between a source with sky coordinates $(l, m)$ and the baseline vector of an interferometer, denoted by $D_\lambda$. The baseline is described by its components $(u, v, w)$. Image from \cite{Thompson}.}
  %\SC{here may be a better image out there.}}
  \label{fig: UV-plane}
\end{figure}
...
%   \caption[\uv coverage for the \bfour observations.]{\uv coverage for the \bfour observations. The arcs result from the rotation of the Earth, with each arc corresponding to a different antenna baseline pair.}
%   %\later{I don't love how this saved, I can add my own axes so its an all white background.} \code{work on this} \SCC{comment}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/Inter & Data/uv_plot.png}
  \caption[\uv coverage for the \bfour observations.]{\uv coverage for the \bfour observations. The arcs result from the rotation of the Earth, with each arc corresponding to a different antenna baseline pair.}
  \label{fig: UV-coverage}
\end{figure}
...
% ------------------------------------------------
To convert the visibilities into an image, we must apply an inverse Fourier transform of the complex visibilities given in Equation \ref{eq: V(u,v)}.  This inverse Fourier transform is \citep{Thompson}:
\begin{equation}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/BeforeAndAfterClean_NOTSELFCAL.png}
  \caption[Dirty vs cleaned image of IRS 63]{Comparison of the dirty (left) and \texttt{clean} \SI images of IRS 63. The dirty image is convolved with the dirty beam, while the \texttt{clean} image is deconvolved with the synthesized beam. Both images have the same intensity scaling. }

  %\question{Do I have access to a pre-clean image of IRS 63 or is this something I will need to get from a different source? (I will add a caption too).} 
...
The signal received by each antenna can be affected by time-dependent atmospheric variations, which can introduce additional calibration errors. Self-calibration can reduce the artifacts caused by these errors by correcting for direction-independent phase and amplitude errors. Similar to gain calibration, self-calibration only corrects the phase and amplitude of the data, and does not correct for bandpass or flux calibration errors \citep{Marvil2}. The specific details on the self-calibration process used in this project are described in Chapter \ref{ch_data}. 
% ------------------------------------------------
