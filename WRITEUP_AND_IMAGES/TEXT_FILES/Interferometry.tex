...
\begin{figure}[htbp]
    ...
    \caption[Schematic of a two-antenna radio interferometer.]{Schematic of a two-antenna ratio interferometer highlighting the time delay between signals received at the two antenna. $B$ is baseline between the two antennas, and the corresponding time delay is $B\cos(\phi)$, where $\phi$ is the angle between the baseline and the direction to the science target.}
...
\begin{figure}[htbp]
  ...
  \caption[\uv coverage for the \bfour observations.]{\uv coverage for the \bfour observations. The arcs result from the rotation of the Earth, with each arc corresponding to to a different antenna baseline pair.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/BeforeAndAfterClean_NOTSELFCAL.png}
  \caption[Dirty vs cleaned image of IRS 63]{\note{THIS IS NOT THE REAL IMAGE! Just a placeholder for formatting purposes.}\question{Do I have access to a pre-clean image of IRS 63 or is this something I will need to get from a different source? (I will add a caption too).} \code{work on this}}
...
The signal received by each antenna can be affected by time-dependent atmospheric variations, which can introduce additional calibration errors. Self-calibration can reduce the artifacts caused by these errors by correcting for direction-independence phase and amplitude errors. Similar to gain calibration, self-calibration only corrects the phase and amplitude of the data, and does not correct for bandpass or flux calibration errors \citep{Marvil2}. The specific details on the self-calibration process used in this project are described in Chapter \ref{ch_data}. 
% ------------------------------------------------
