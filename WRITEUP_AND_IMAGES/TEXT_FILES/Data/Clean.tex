...
Additionally, the \SQ and \SU data were cleaned using \texttt{tclean}, using the final mask from the amplitude calibration at its respective wavelength. The same Briggs \texttt{robust} value used for \SI was used for \SQ and $U$, with \texttt{nterms = 1}.  \sudo{Checked this for Band 4 and 7. Can check 5 and 7 when I can access my data again. "Please do" - Sarah}
...
\begin{tablenotes}
\footnotesize
\item \textbf{Note:} \SIerrNS, \SQerrNS, \SUerr are constant values for \bfourNS, \bfiveNS, and \bsevenNS. For \bsixNS, the uncertainties were provided as maps; the values reported in this table correspond to the pixel with the maximum \SI value. \sudo{"Are the Band 5 data that much worse in terms of sensitivity to Band 7?  It could be possible, but please check." - Sarah, note: I will check this when we sudo reboot}
\end{tablenotes}
