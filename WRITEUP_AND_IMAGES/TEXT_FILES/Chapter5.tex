...
\begin{figure}[htbp]
  ...
  \caption[Absorption and effective scattering mass opacities for compact and porous grains.]{Same as Figure \ref{fig: kappa_vs_lambda}, but for compact (left) and porous (right) grains. Blue box corresponds to lower limit of \bseven and upper limit of \bfourNS.}
...
\noindent Figure \ref{fig: Kataoka2015Figure3Recreation} shows this \Pweff curve is shown is as a black dashed line, which has a distinctive peak, showing where the balance between polarization is most efficient.
...
\noindent Table \ref{tab: polf_comparison} summaries the values from these three methods at each wavelength. 
...
Table \ref{tab: polf_comparison} shows that \POLFpoli has the highest values, which could be biased high due to the forward scattering bias observed in Chapter \ref{sec: ch4_intensity_profiles}. The Gaussian method, \POLFgaussNS, produces the lowest polarization fraction at each wavelength. This may indicate that it is biased low due to depolarization from unresolved polarization structure within the beam, as it averages the polarization over multiple pixels rather just measuring a single pixel. The \POLFsi method therefore provides the best representation of the observed polarized fraction, and is used throughout the main analysis.
...
\begin{equation}
\chi_\mathcal{P}^2 = \sum_{\lambda = all\text{ }wavelengths} \left(\frac{\POLFsiEQ(\lambda) - \PwmodelEQ(\lambda, \amaxbestEQ)}{\POLFerrEQ (\lambda)} \right)^2.
...
\begin{figure}[htbp]
  ...
  \caption[Curves of \Pwmodel to compare to \POLFsiNS.]{Curves of \Pwmodel (Equation \ref{eq: P_SF}) to compare to \POLFsiNS. Markers are \POLFsiNS, listed in Table \ref{tab: polf_comparison}. The best-fit (lowest $\chi^2$, Equation \ref{eq: chi_squared_dust_model}) is represented with a gold star. The specific results of the fit can be found in Table \ref{tab: dust_model_fits_spherical}. Wavelengths that are not included in the fit for the SF have a dashed line. }
