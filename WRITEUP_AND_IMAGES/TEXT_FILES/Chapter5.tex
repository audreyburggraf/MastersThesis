...
$P$  & Polarization fraction predicted by the model at $\Theta$ = 90\degg & \% \\
...
The multiplication of these curves (red and blue) produces the effective polarized scattering fraction, \PweffNS, which is shown as a black dashed line.  The \Pweff curve has a distinctive peak, showing where the balance between polarization is most efficient.
...
\noindent Table \ref{tab: polf_comparison} summaries the values from these three methods at each wavelength. $\mathcal{P}_{\mathrm{F, max\,\POLIeq}}$ has a slight bias due to the forward scattering bias observed in Chapter \ref{sec: ch4_intensity_profiles}. $\mathcal{P}_{\mathrm{F, Gaussian}}$ produces similar results at higher porosities, but the goodness-of-fit significantly drops at lower porosities, potentially because the polarized fraction does not follow the wavelength-dependent trend observed in the other two methods.  \SCC{comment} Therefore, \POLFsi is the best representation of the polarization fraction per wavelength, and is used throughout the rest of the analysis. 
