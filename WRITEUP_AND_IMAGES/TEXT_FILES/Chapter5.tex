...
$P_{90}$  & Polarization fraction predicted by the model at $\Theta$ = 90\degg & \% \\
\Pweff & Effective polarized scattering fraction & \% \\
$f$  & Filling factor & -- \\
\POLFsi & Polarization fraction at the pixel with maximum Stokes $I$ & \% \\
\sfbest  & Best-fit scale factor & -- \\
\Pwmodel & Scaled effective polarized scattering fraction, \PweffNS \sfbestNS & \% \\
$\chi^2_{\mathrm{P}}$ & Reduced chi-squared statistic for the polarization fraction fit & -- \\
\bottomrule
\end{tabular}
\begin{tablenotes}
% \footnotesize
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.34\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/NOTMINE/glitterinFigure1.png}
  \caption[Example of six irregular particles from \texttt{glitterin}.]{Example of six irregular particles from \texttt{glitterin}. Figure from \cite{glitterin}, reproduced with permission.}
...
\end{enumerate}
\noindent Geometric scattering applies when the grain is much larger than the observing wavelength. In this regime, the interaction can be approximated by geometrical optics, where the radiation is treated as rays that are either absorbed or deflected by reflection and refraction \citep{Bohren1983}.
% ----------------------------------------------
...
% ----------------------------------------------
Rather than using Mie theory, which is for spherical grains, \texttt{glitterin} uses a neural network trained to predict light scattering from irregularly shaped dust grains. Predictions are made based on input parameters such as the size parameter. 
...
\label{sec: ch5_opticalproperties}
...
\end{equation}
\noindent where $n$ and $k$ are the real and imaginary parts of the refractive index, respectively \citep{Zhang2023}. These values are calculated for each observing wavelength using \texttt{DSHARP} and do not vary with porosity or grain shape. 
...
The \texttt{glitterin} functions do not directly calculate \kabs and \kscaNS, but they calculate the absorption and scattering cross sections, $C_\text{abs}$ and $C_\text{sca}$. These can be used to calculate \kabs and \kscaNS, given by:
\begin{equation}
    \kappa_i = \frac{C_i}{\frac{4\pi}{3}\rho_s r_\text{vol}^3},
\end{equation}
\noindent where $i$ can be abs or sca, and $\rho_s$ is the density of a compact grain with DSHARP dust composition. To calculate the $C_i$ values with \texttt{glitterin}, it requires the user to specify \xvolNS, $n$, and $k$, which were calculated using \texttt{DSHARP}, the observing wavelength, and the scattering angle (defined below). 
...
% g is a parameter of how strongly scattering prefers the forward direction: g = 0 (isotropic, equally likely to scatter in any direction), g > 0 (prefers forward), etc 
...
\end{equation}
\noindent Figure \ref{fig: Kataoka2015Figure1Recreation} shows the absorption and effective scattering mass opacities as a function of observing wavelength for maximum grain sizes of \amax = 1 \micron and 100 \micron for compact and porous grains. 
...
\begin{figure}[htbp]
  ...
  \caption[Absorption and effective scattering mass opacities for compact and porous grains.]{Same as Figure \ref{fig: kappa_vs_lambda}, but for compact (left) and porous (right) grains. The blue box corresponds to the lower limit of \bseven and the upper limit of \bfourNS.}
...
An additional quantity that can be calculated for spherical and irregular grains is the effective single scattering albedo, which describes the fraction of the original light that is removed by scattering rather than absorption. This is given by the equation \citep{Zhang2023}: 
\begin{equation}
    \weffEQ = \frac{\kscaeffEQ}{\kabsEQ + \kscaeffEQ}.
    \label{eq: omega}
\end{equation}
\noindent This will be explored more in the following subsection. 
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/DustModels/Kataoka2015Fig2_grid.pdf}
...
\begin{figure}[htbp]
  ...
  \caption[Polarization fraction $P_{90}$ and effective albedo as a function of grain size.]{Plot of polarization fraction $P_{90}$ in red and effective albedo in blue, and their produce, \PweffNS,  as a function of grain size for compact (left) and porous (right) dust grains. The characteristic grain size for each wavelength is overplotted in grey, highlighting that it roughly corresponds to the local maxima of both curves. }
...
The multiplication of these curves (red and blue) produces the effective polarized scattering fraction:
\begin{equation}
    \PweffEQ = P_{90} \times \weffEQ.
\end{equation}

\noindent Figure \ref{fig: Kataoka2015Figure3Recreation} shows this \Pweff curve as a black dashed line, which has a distinctive peak, showing where the balance between polarization is most efficient.





The \Pweff product provides a measure of the contribution of each grain size to the polarized scattered light at a specific observing wavelength, and can therefore be used to determine the grain sizes that contribute most strongly to the observed polarization \citep{Kataoka_2015, Tazaki}. As mentioned earlier, \achar is the grain size at which dust self-scattering is most efficient, so \Pweff is expected to peak near the characteristic grain size. 
Figure \ref{fig: Kataoka2015Figure3Recreation} shows \achar for each wavelength as a vertical grey line. For compact grains, the \Pweff peak occurs near \acharNS, as expected, highlighting the dependence of the polarization scattering efficiency on observing wavelength.

For porous grains (right panels of Figure \ref{fig: Kataoka2015Figure3Recreation}), the overall wavelength-dependent behaviour of the polarization and effective albedo remains similar, but their dependence on grain size is less sharply peaked, with both parameters remaining close to the maximum for a broader range of grain sizes. The grain size at which the polarization fraction decreases and effective albedo increases is shifted toward larger grain sizes, causing the \Pweff curve to peak at a larger grain size than \acharNS. Additionally, the \Pweff peak is wider than the sharp peaks for compact grains. Together, these differences indicate that porous grains can efficiently contribute to polarized scattering at larger grain sizes and over a wider range of grain sizes compared to compact grains. This behaviour is assumed to be related to different scattering properties of porous grains due to their lower density and more empty space within the grains. 
...
Table \ref{tab: amax_peak_values} lists the grain sizes at which \Pweff reaches its maximum (\apeakNS), as well as the characteristic grain size at each wavelength. The \apeak values occur at grain sizes of approximately hundreds of microns, indicating that polarization scattering is most sensitive to grains in this range. By combining observations at multiple wavelengths, the grain size dependence can be utilized to provide a stronger constraint on the maximum grain size. The goal of this chapter is therefore to utilize the wavelength and grain size dependence of polarized scattering to determine the maximum grain size, shape, and porosity that best reproduces the observations. How the models are compared with the multi-wavelength observations is described in Chapter \ref{sec: ch5_determining_abest}.
...
\begin{enumerate}
    ...
    \item \POLFpoliNS: The polarization fraction at the pixel with maximum polarized intensity.
...
\noindent Table \ref{tab: polf_comparison} summarizes the values from these three methods at each wavelength. 
...
Table \ref{tab: polf_comparison} shows that \POLFpoli has the highest values, which could be biased high due to the forward scattering bias observed in Chapter \ref{sec: ch4_intensity_profiles}. The Gaussian method, \POLFgaussNS, produces the lowest polarization fraction at each wavelength. This may indicate that it is biased low due to depolarization from unresolved polarization structure within the beam, as it averages the polarization over multiple pixels rather than just measuring a single pixel. The \POLFsi method therefore provides the best representation of the observed polarized fraction, and is used throughout the main analysis.


% Gaussian: Spherical Table Table \ref{tab: dust_model_fits_spherical_Gaussian}


%POLI: Spherical Table \ref{tab: dust_model_fits_spherical_POLI}

% $\mathcal{P}_{\mathrm{F, Gaussian}}$ produces similar results at higher porosities, but the goodness-of-fit significantly drops at lower porosities, potentially because the polarized fraction does not follow the wavelength-dependent trend observed in the other two methods. 



% So, \POLFsi is used throughout the main analysis, while the results from the other two methods are presented in \ref{sec: appendix_maxPOLI} and \ref{sec: appendix_Gaussian} for $\mathcal{P}_{\mathrm{F, max\,\POLIeq}}$ and $\mathcal{P}_{\mathrm{F, Gaussian}}$, respectively, to explore the effect of the other polarization fraction methods on the fitting results. (note if things change with method)
...
% ----------------------------------------------
The polarization fraction (\POLFsiNS) and the \Pweff values from spherical and irregular grains cannot be directly compared because they represent different quantities. The observed polarization fraction is a dimensionless ratio of polarized intensity to total intensity, while \Pweff is the polarization scattering efficiency of the dust. A scale factor (SF) is therefore introduced to relate the model values to observations. This scaling is necessary because not all of the observed emission is due to scattering. Thermal emission also contributes to the observed emission.

The scale factor is calculated at each \amax in the sample, given by:
\begin{equation}
\sfeq(\amaxEQ) = \frac{\POLFsiEQ}{\PweffEQ(\amaxEQ)}.
    \label{eq: sf}
\end{equation}


\noindent This provides a scale factor for each wavelength included in the fit at every value of \amaxNS. If a given grain size provides a good reproduction of the observations across all fitted wavelengths, the resulting scale factors should be similar between the wavelengths. For each value of \amaxNS, the standard deviation of the scale factors is calculated to determine the spread of the scale factors between wavelengths using \texttt{numpy.std}. 


The best-fit grain size (\amaxbestNS) is defined as the value of \amax for which the standard deviation of the scale factors is minimized. The corresponding scale factor is the best-fit scale factor (\sfbestNS). The \Pweff values are then multiplied by the \sfbest to obtain the scaled model polarization (\PwmodelNS), allowing the model and observations to be directly compared:
\begin{equation}
    \PwmodelEQ = \PweffEQ \times \sfbestEQ. 
    \label{eq: P_SF}
\end{equation}
...
Multiple wavelength combinations were tested to determine the effect of each wavelength of data on the success of the fit. The fits were (1) all wavelengths, (2) no \bfiveNS, and (3) just \bfour and \bsevenNS. Case (2) was tested due to issues with the \bfive data, explored in Chapter \ref{sec: discussion_band5_data_issues}.  A reduced chi-squared value was calculated for each fitting case to determine the agreement between the model and observations, given by: 
\begin{equation}
\chi_\mathcal{P}^2 = \frac{1}{N_\lambda - 1} \times \sum_{\lambda = all\text{ }wavelengths} \left(\frac{\POLFsiEQ(\lambda) - \PwmodelEQ(\lambda, \amaxbestEQ)}{\POLFerrEQ (\lambda)} \right)^2,
    \label{eq: chi_squared_dust_model}
\end{equation}

\noindent where $N_\lambda - 1$ represents the degrees of freedom, $N_\lambda$ is the number of wavelengths included in the fit and 1 is the number of free parameters (\amaxNS). The calculation is summed over all wavelengths, as each wavelength was included in the $\chi_\mathcal{P}^2$ calculation, regardless of whether it was included in the fit for the scale factor value or not. The fitting combination (e.g., each porosity or irregular dust model) that produced the lowest $\chi_\mathcal{P}^2$ value was chosen to be the best-fit model.
% ----------------------------------------------
% ----------------------------------------------
...
% ----------------------------------------------
Table \ref{tab: dust_model_fits_spherical} reports the results of the spherical dust models. For each fitting case, the best-fit scale factor from Equation \ref{eq: sf}, the corresponding $\chi_\mathcal{P}^2$ value (Equation \ref{eq: chi_squared_dust_model}), and the best-fit grain size (\amaxbestNS) are reported. The best fit for each fitting case is shown in bold. Figure \ref{fig: Spherical_BEST} shows the best overall fitting case (all wavelengths) for the porosities focused on in this research, with the additional porosities shown in Figure \ref{fig: appendix_AllBands_extra_porosities}. Equivalent plots for the other fitting cases are provided in Appendicies \ref{appendix: max Stokes I wo5} and \ref{appendix: no 4 or 7}. The observed polarization fraction, \POLFsiNS, is shown as a colour-matched marker for each wavelength. 
...
\begin{table}[h!]
\centering
\caption[Spherical dust grain model fitting results.]{Spherical dust grain model fitting results, including the scale factor, $\chi_\mathcal{P}^2$ and best-fit grain size. }
\begin{tabular}{lcccc}
\toprule
...
& 0.75 & 2.11 & 2.34 & 200 \\
& 0.60 & 2.10 & 0.82 & 225 \\
& \textbf{0.50} & \textbf{2.13} & \textbf{0.46} & \textbf{244} \\
& 0.30 & 2.26 & 0.89 & 287 \\
& 0.25 & 2.33 & 1.06 & 301 \\
& 0.10 & 2.95 & 2.11 & 360 \\
& 0.01 & 9.40 & 12.52 & 512 \\
...
& 0.75 & 2.20 & 5.62 & 202 \\
& 0.60 & 2.16 & 1.90 & 227 \\
& \textbf{0.50} & \textbf{2.15} & \textbf{0.66} & \textbf{245} \\
& 0.30 & 2.27 & 1.09 & 290 \\
& 0.25 & 2.34 & 1.54 & 303 \\
& 0.10 & 2.97 & 2.78 & 362 \\
& 0.01 & 9.39 & 19.01 & 509 \\
...
\begin{figure}[htbp]
  ...
  \caption[Curves of \Pwmodel to compare to \POLFsi for the all wavelengths fit case.]{Curves of \Pwmodel (Equation \ref{eq: P_SF}) to compare to \POLFsi for the all wavelengths fit case. Markers are \POLFsiNS, listed in Table \ref{tab: polf_comparison}. The best-fit (lowest $\chi_\mathcal{P}^2$, Equation \ref{eq: chi_squared_dust_model}) is represented with a gold star. The specific results of the fit can be found in Table \ref{tab: dust_model_fits_spherical}. Wavelengths that are not included in the fit for \sfbest have a dashed line. }
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{WRITEUP_AND_IMAGES/IMAGES/WRITEUP/DustModels/chi_fs_f_reduced.pdf}
  \caption[Plot of filling factor, $f$, versus the goodness-of-fit parameter, $\chi_\mathcal{P}^2$.]{Plot of filling factor, $f$, versus the goodness-of-fit parameter, $\chi_\mathcal{P}^2$, showing a local minimum at $f = 0.5$ for all fitting cases for spherical grains.  } 
  \label{fig: chi_vs_f}
\end{figure}
% ----------------------------------------------------------------
...
All wavelengths  & \textbf{1.83} & \textbf{1.51} & \textbf{209} \\
No \bfive               & 1.88 & 2.96 & 211 \\
...
\begin{figure}[htbp]
  ...
  \caption[Same as Figure \ref{fig: Spherical_BEST} but for irregular dust grains.]{Same as Figure \ref{fig: Spherical_BEST} but for irregular dust grains. Porosity is fixed, and the grid shows the results for each fitting case. Corresponding results can be found in Table \ref{tab: dust_model_fits_irregular}.  } 
