...
\begin{figure}[htbp]
  ...
  \caption{Two examples of the Gaussian weights for selected values of $\phi$, $a$ and $b$. \coded{() units to []}}
...
\begin{figure}[htbp]
  ...
  \caption{Same as Figure \ref{fig: RatioBestGrid}, but for the best-fit Gaussian models. The corresponding best-fit Gaussian models are listed Table \ref{tab:gaussian_top5}. \coded{() units to []}}
...
\begin{figure}[htbp]
  ...
  \caption{Maps of the Gaussian uniform weights $W_{\text{uniform}}$ (Equation \ref{eq: gaussian_eq}) for the best-fit Gaussian model for each wavelength, corresponding vector pattern shown in Figure \ref{fig: GaussianBestGrid}. \coded{() units to []}}
