...
\end{equation}
\noindent where $\lambda$ is the observing wavelength \citep{Zhang2023}. When the size parameter is equal to 1, this is defined as the characteristic grain size ($a_{\text{char}}$), equal to $\lambda/2\pi$. 
...
The dust models were calculated using the DSHARP Mie-Opacity Library (\texttt{DSHARP}) from \cite{dsharp_code}. 
% ----------------------------------------------
...
\question{Add more detail on how the \texttt{DSHARP} code incorperates different filling factors?}
...
An additional quantity is the single scattering albedo, which describes the fraction of the original light that is removed by scattering rather than absorption. The standard and effective albedo are defined as:
\begin{equation}
